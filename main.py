...
<body>
    ...
    function renderGrades() {
      ...
      if (grades.length > 0) {
        ...
        container.innerHTML = '<p style="color: #64748b;">No completed student grades yet.</p>';
      }
    }
...
<body>
    ...
    function initStudentPortal() {
        const savedStore = localStorage.getItem('soroban_assignments_store');
        if (savedStore) {
            try { masterStore = JSON.parse(savedStore); } catch(e) { masterStore = HARDCODED_10_PROBLEMS; }
        } else {
            masterStore = HARDCODED_10_PROBLEMS;
            localStorage.setItem('soroban_assignments_store', JSON.stringify(masterStore));
        }
        ...
        if (activeId) {
            currentWorksheet = masterStore.find(item => String(item.id) === String(activeId));
            if (currentWorksheet) {
